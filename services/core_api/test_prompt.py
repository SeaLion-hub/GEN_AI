...
# .env 파일에서 OPENAI_API_KEY 로드 (services/core_api/.env)
# 이 스크립트는 core_api 폴더에서 실행되어야 합니다.
load_dotenv() 
...
# --- B님이 만든 20개 테스트 데이터셋 ---
# (Primary/Secondary 절충안에 맞춰 ground_truth를 수정했습니다)
TEST_DATASET = [
    # (유형 1: FOMO / 추격 매수)
    ...
    # (유형 5 + 3 복합 - B님의 절충안을 테스트할 핵심 데이터)
...
def run_prompt_test():
    ...
    for i, test_case in enumerate(TEST_DATASET):
        ...
        # 'ground_truth'는 AI에게 보내지 않도록 분리
        ...
        # AI 함수 호출
        ai_response = get_ai_feedback(test_case)
        ...
        # AI가 "null", "none", "" 등으로 응답할 경우를 대비해 None으로 표준화
